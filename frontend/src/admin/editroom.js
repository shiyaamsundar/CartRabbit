...
import { addroom, getroomdet,updateroom} from './adminapi'
...
const Editroom=({match})=>{
  ...
  const successmessage=()=>{
    ...
    style={{display:didredirect?"":"none"}}>
      New Room has been created Successfully by {name}  <Link to="/admin/dashboard">Click here</Link> to go back to the dashboard
    </div>
    )
  }
...
//
  const onsubmit=(event)=>{
    ...
    updateroom(user.id,token,match.params.roomid,values).
    then(data=>{
      console.log(user)
        if(data.error){
    ...
    const adminform=()=>(
      ...
      <div className="row">
        ...
        <button  onClick={onsubmit} className="btn btn-outline-info ">Save Changes</button>
      </div>
    </form>
    )
    const goBack = () => (
